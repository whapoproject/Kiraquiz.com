...
def user_login(request):
    ...
    return render(request, 'realacc/user_login.html', {'error_message': error_message})
        ...
        if 'deposit_amount' in request.POST:
            ...
            if deposit_amount:
                ...
                if deposit_amount > 0:
                    ...
                    return redirect('real_account')
    return render(request, 'realacc/deposit.html', {
        'user_profile': user_profile,
        'account': account,
        'deposits': deposits,
        'withdraws': withdraws,
...
from django.shortcuts import render, redirect
from .models import UserProfile, Account, Deposit, Withdraw, Pay_quizz, Paid_quizz

def pay_with(request):
    user_profile = UserProfile.objects.get(user=request.user)
    account = Account.objects.get(user=request.user)
    deposits = Deposit.objects.filter(user=request.user)
    withdraws = Withdraw.objects.filter(user=request.user)
    ...
    return render(request, 'realacc/with.html', {
        'user_profile': user_profile,
        'account': account,
        'deposits': deposits,
        'withdraws': withdraws,
...
def payment(request):
    account = Account.objects.get(user=request.user)
    # Calculate the total Profite for the user's earnings
        ...
        if 'payquizzes_amount' in request.POST:
            ...
            if payquizzzes_amount:
                ...
                if payquizzzes_amount > 0 and payquizzzes_amount <= account.balance:
                    # Update the user's balance after payment
                    account.balance -= payquizzzes_amount
                    account.save()

                    # Record the payment in the Pay_quizz model
                    Pay_quizz.objects.create(user=request.user, amount=payquizzzes_amount)

                    return redirect('quiz_view')
                else:
                    error_message = "Insufficient balance. Please deposit or recharge your balance."
        ...
        if 'payquizzes_amount' in request.POST:
            ...
            if payquizzzes_amount:
                ...
                if payquizzzes_amount > 0 and payquizzzes_amount <= account.balance:
                    ...
                    error_message = "Insufficient balance.Please Select under level or recharge your balance."
